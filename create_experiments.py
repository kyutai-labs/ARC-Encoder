...
def main(args):
    ...
    if args.prefix == 'lr':
        ...
        config['wandb']['run_name'] = args.prefix + str(args.proj_n_layers) + args.llm_name
    
    elif args.prefix == 'hidden_dim':
        config['exp_name'] = args.prefix + str(args.proj_hidden_dim) + args.llm_name
        config['wandb']['run_name'] = args.prefix + str(args.proj_hidden_dim) + args.llm_name
        ...
        config['wandb']['run_name'] = args.prefix + str(args.norm_wo_embeds) + args.llm_name
    
    else :
        config['exp_name'] = args.prefix + args.llm_name +  sha1(name.encode("utf8")).hexdigest()[:20]
        config['wandb']['run_name'] = args.prefix + args.llm_name + sha1(name.encode("utf8")).hexdigest()[:20]
    
        
    if args.llm_name == 'Mistral7B':
        with open(f'/home/hippolytepilchen/code/embed_llm/config/experiments/mistral/{config["exp_name"]}.yaml', 'w') as file:
            yaml.dump(config, file)
    elif args.llm_name == 'Gemma7B':
        with open(f'/home/hippolytepilchen/code/embed_llm/config/experiments/gemma/{config["exp_name"]}.yaml', 'w') as file:
            yaml.dump(config, file)
    elif args.llm_name == 'Llama3.2-3B':
        with open(f'/home/hippolytepilchen/code/embed_llm/config/experiments/llama/{config["exp_name"]}.yaml', 'w') as file:
            yaml.dump(config, file)
    else:
        raise ValueError(f'{args.llm_name} not supported yet !')
...
if __name__ == '__main__':
    ...
    #         elif config['llm_name'] == 'Llama3.2-3B':
