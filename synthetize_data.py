    ...
    for sample in dataset:
        if not adapt_seq_len:
            ...
            for i, s in enumerate(splitted_sample):
                if i == 0:
                    tok_seq = tokenizer.instruct_tokenizer.tokenizer.encode(
                        s, bos=True, eos=False
                    )
                else:
                    tok_seq = tokenizer.instruct_tokenizer.tokenizer.encode(
                        s, bos=False, eos=False
                    )
        ...
        if args.instruct_model:
            ...
            completion_request = ChatCompletionRequest(
                messages=[UserMessage(content=passages[0] + instruction)]
            )
            # encode message
            tokens = tokenizer.encode_chat_completion(completion_request).tokens
            batch_list.append(
                BatchSample(passages=passages, question=instruction, tokens=tokens)
            )
        else:
            batch_list.append(
                BatchSample(
                    ...
                    tokens=passage
                    + tokenizer.instruct_tokenizer.tokenizer.encode(
                        instruction, bos=False, eos=False
                    ),
                )
            )
...
def main(args):
        ...
        if args.num_gen is None:
            for rank in range(1, get_world_size()):
                (
                    ...
                    / args.output_file.replace(".jsonl", str(rank) + ".jsonl")
                ).touch()
        else:
            (
                Path(args.output_path)
                / args.output_file.replace(".jsonl", str(args.num_gen) + ".jsonl")
            ).touch()
    ...
    data_loader = dataloader_from_file(
        args.data_path,
        args.batch_size,
        mistral_tokenizer,
        args.seq_sizes,
        num_gen=args.num_gen,
        overall_gen=args.overall_gen,
    ...
    for step in range(args.max_steps):
        ...
        out_tokens, _, is_a_nan = generate(
            tokens,
            model,
        ...
        if is_a_nan:
            ...
            continue
        max_token_size = max(max_token_size, max([len(t) for t in tokens]))
        truncated_list = []
        ...
        for j in range(args.batch_size):
            result = mistral_tokenizer.instruct_tokenizer.tokenizer.decode(
                truncated_list[j]
            )
            data_buffer.append(
                {
                    "passages": batch[j].passages,
        ...
        n_toks += torch.tensor(sum([len(t) for t in truncated_list])).item()
        if step % 100 == 0:
            if is_torchrun():
                buffer_data = torch.tensor(
                    [n_data.item()], dtype=torch.int32, device=f"cuda:{local_rank}"
                )
                buffer_toks = torch.tensor(
                    [n_toks.item()], dtype=torch.int32, device=f"cuda:{local_rank}"
                )
                dist.all_reduce(buffer_data, op=dist.ReduceOp.SUM)
                dist.all_reduce(buffer_toks, op=dist.ReduceOp.SUM)
                if get_rank() == 0:
                    print(
                        f"Step {step} took {time.time() - start_time} seconds",
                        "Data processed:",
                        buffer_data[0].item(),
                        "Tokens generated:",
                        buffer_toks[0].item(),
        ...
        if step % args.freq_load_data == 0 and step != 0:
            ...
            if args.num_gen is None:
                with open(
                    Path(args.output_path)
                    / args.output_file.replace(".jsonl", str(get_rank()) + ".jsonl"),
                    "a",
                ) as f:
...
def arg_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="A list of integers (space separated)",
