...
def dataloader_from_file(
    ...
    while True:
        exs = random.choice(["hotpot", "nq", "trivia", "squad", "dont_know"])

        if exs == "squad" or exs == "dont_know":
            instruction_prompts = random.choice(instruction_prompts_wodoc).format(
                examples=SQUAD_EXAMPLE
            )
            passage = None
        else:
            passage = next(dataset)
            instruction_prompts = random.choice(instruction_prompts_wdoc).format(
                examples=(
                    HOTPOT_EXAMPLE
                    if exs == "hotpot"
                    else NQ_EXAMPLE
        ...
        batch_list.append(
            Batch(instruction_prompts=instruction_prompts, passage=passage)
        )
...
def synthesize_data(
    ...
    out_file_path = (
        output_path + "synthesized_" + model_folder_path.split("/")[-1] + ".jsonl"
    )
    ...
    for step in range(max_steps):
        ...
        text_prompts = [b.instruction_prompts for b in batch]
        outputs = llm.generate(text_prompts, sampling_params)
        for i, output in enumerate(outputs):
            if output.finished:
                if batch[i].passage is not None:
                    passage = batch[i].passage
                    generated_text = output.outputs[0].text.strip()
                    if (
                        "Question:" not in generated_text
                        or "Answer:" not in generated_text
                    ):
                        print("No Question or Answer found, skipping this example")
                        continue
                    question = generated_text.split("Question:")[-1]
                    answer = generated_text.split("Answer:")[-1].strip()
                    question = question.split("Answer:")[0].strip()
                else:
                    if "Document:" not in output.outputs[0].text:
                        print("No Document found, skipping this example")
                        continue
                    generated_text = (
                        output.outputs[0].text.split("Document:")[-1].strip()
                    )
                    if (
                        "Question:" not in generated_text
                        or "Answer:" not in generated_text
                        ...
                        print("No Question or Answer found, skipping this example")
                        continue
                    passage = generated_text.split("Question:")[0].strip()
                    question = generated_text.split("Question:")[-1]
                    question = question.split("Answer:")[0].strip()
