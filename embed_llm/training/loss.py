...
def compute_kl_loss_with_mask(
    rag_logits: torch.Tensor, pred_logits: torch.Tensor, rag_mask: torch.Tensor, pred_mask: torch.Tensor, temp: float = 1.0
):
    assert torch.sum(rag_mask.int()) == torch.sum(pred_mask.int()), "Mask should be the same for both logits."
    ...
    mb_loss = loss_func(F.log_softmax(pred_logits / temp, dim=-1), F.softmax(rag_logits / temp, dim=-1))
    return mb_loss