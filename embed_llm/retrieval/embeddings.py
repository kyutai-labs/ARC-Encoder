...
def get_embedder(model_name: str):
    if model_name == "GritLM":
        ...
        model = AutoModel.from_pretrained(
            'facebook/contriever', device_map='auto')
        return model
...
def generate_embeddings(model_name: str, output_path: str, bs: int, dataset, n_gpu: int = 1, partition: int = 0, checkpoint: int = 1000):
    ...
    for ind, i in tqdm(enumerate(range(0, len(used_texts), bs))):
        ...
        if ind % (checkpoint) == 0 and ind != 0:
            ...
            np.save(os.path.join(output_path, model_name,
                    f"{partition}_embeddings_{count}.npy"), embeddings_array)
            with open(os.path.join(output_path, model_name, f"{partition}_embeddings_{count}.jsonl"), 'w') as f:
