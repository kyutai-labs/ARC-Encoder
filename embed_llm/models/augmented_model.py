...
def pad_and_convert_to_tensor(
    ...
    final_x = torch.ones((len(sizes),seq_len), dtype=torch.long).cuda(non_blocking=True) * pad_id
    final_y = torch.ones((len(sizes),seq_len), dtype=torch.long).cuda(non_blocking=True) * pad_id
    final_mask = torch.zeros((len(sizes),seq_len), dtype=torch.bool).cuda(non_blocking=True)
...
class EmbedAugModel(nn.Module):
    def __init__(self, model_name: str, mlp_project_args: MLPProjectArgs, llm: FullyShardedDataParallel,
        ...
        if 'mistral' in self.llm_name:
            self.forward = partial(self.forward_mistral,norm_wo_embeds = norm_wo_embeds)
    ...
    def forward_mistral(self, batch: Batch,  norm_wo_embeds: bool):    
        ...
        with torch.no_grad():
            embeddings = encode_text(batch.texts, self.embed_model_name, self.embedding_model, query_embedding=False, device='cuda').cuda()
    ...
    def forward_gemma(self, batch: Batch, max_seq_len: int, pad_token_id: int, norm_wo_embeds: bool):
        ...
        att_mask = torch.full((max_seq_len, max_seq_len), float("-inf")).cuda(non_blocking=True)
    ...
    def load_inference_model(
        ...
        if 'mistral' in model_name.lower():
            model = MistralTransformer.from_folder(Path(llm_path), max_batch_size, max_seq_len, device, variant)
            model.eval()
            tokenizer =  load_mistraltokenizer(Path(llm_path)).instruct_tokenizer.tokenizer
        ...
        if mlp_path is not None:
            ...
            augmented_model = EmbedAugModel(model_name, mlp_project_args, model, embed_model_name, embedding_model, norm_wo_embeds, tokenizer)
            augmented_model.mlp_project.load_state_dict(torch.load(Path(mlp_path) / "model.pt"))
            
        augmented_model.eval()
        if 'mistral' in model_name.lower():
            augmented_model.generate = augmented_model.generate_mistral
        elif 'llama' in model_name.lower():
            augmented_model.generate = partial(augmented_model.generate_llama, llama_model = Llama(llm, tokenizer))             
        elif 'gemma' in model_name.lower():
            augmented_model.generate = partial(augmented_model.generate_gemma, device = device)
    ...
    def generate_gemma(
        ...
        embeddings = encode_text(text_conditioning, self.embed_model_name, self.embedding_model, query_embedding=False, device='cuda')
    ...
    def generate_mistral(self, 
        ...
        embeddings = encode_text(text_conditioning, self.embed_model_name, self.embedding_model, query_embedding=False, device='cuda')
        encoded_prompts = [self.tokenizer.encode(prompt, bos=True, eos=False) for prompt in prompts]
        ...
        produced_text = [self.tokenizer.decode(generated_tokens[i]) for i in range(len(generated_tokens))]
