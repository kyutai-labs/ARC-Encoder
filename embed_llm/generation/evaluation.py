...
def evaluate_model(run_name: str):
    ...
    # If file not found
        ...
        try: 
            pipeline: EmbedAugPipeline = EmbedAugPipeline.load_inference_model(llm_path = llm_path, 
                                                                    ckpt_path = '/lustre/scwpod02/client/kyutai-interns/hippop/tmp/' + run_name + '/checkpoints/checkpoint_008500', 
                                                                    device = device,
                                                                    llm_name = 'Mistral7B' , 
                                                                    embed_model_name = 'NVEmbed', # Not used if pretrainde ckpt available
                                                                    max_batch_size = max_batch_size)
    ...
    for temp in results_generation.keys():
        ...
        for prompt_type in results_generation[temp]['valid'].keys():
            generated_sequences = results_generation[temp]['valid'][prompt_type]['seq']
            if prompt_type == 'empty_prompt':
                ...
                em = np.mean(np.array([get_em(gt, pred) for gt, pred in zip(gt_passage, generated_sequences)]))
                ...
                bleu_score = get_bleu_score(gt_passage, generated_sequences)
                em = np.mean(np.array([get_em(gt, pred) for gt, pred in zip(gt_passage, generated_sequences)]))
            print(f'Temperature: {temp}, Split: valid, Prompt Type: {prompt_type}, Overlap: {overlap}', 'Bleu Score:', bleu_score, 'EM:', em)
            metrics.append({'temp': temp, 'split': 'valid', 'prompt_type': prompt_type, 'overlap': overlap, 'bleu_score': bleu_score, 'em': em})
                    
    with open('/lustre/scwpod02/client/kyutai-interns/hippop/tmp/' + run_name+'/results_generation.json', 'w') as f:
        json.dump(metrics, f)
    
    with open('/home/hippolytepilchen/code/embed_llm/config/experiments/mistral/overall_results.json', 'r') as f:
        overall_results = json.load(f)
    overall_results[run_name] = metrics
    with open('/home/hippolytepilchen/code/embed_llm/config/experiments/mistral/overall_results.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    run_names = os.listdir('/home/hippolytepilchen/code/embed_llm/config/experiments/mistral')
    with open('/home/hippolytepilchen/code/embed_llm/config/experiments/mistral/overall_results.json', 'r') as f:
        overall_results = json.load(f)
