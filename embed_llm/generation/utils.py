...
def format_results(results: dict, benchmark: str):
    ...
    if benchmark.lower() == "nq" or benchmark.lower() == "triviaqa":
        key_list =  ["run_name", "ckpt", "temp", "n_samples","icl_examples", "context_in_examples","context_w_query", "EM Metric", "EM approx_Metric", "F1", "Prop_a_in_cont","n_passages"]
    elif benchmark.lower() == 'reconstruction':
        key_list = ["run_name", "ckpt", "temp", "n_samples", "eval_data_type", "Bleu", "Trunc Bleu", "AVG Bleu", "Meteor", "EM", "Overlap"]
    else:
    ...
    for run_name in results.keys():
        for ckpt in results[run_name].keys():
            if benchmark.lower() == 'reconstruction':
                for metric in ["Bleu", "Trunc Bleu", "AVG Bleu", "Meteor", "EM", "Overlap"]:
                ...
                formated_results = formated_results.groupby(["run_name", "ckpt", "temp", "n_samples", "eval_data_type"]).first().reset_index()
