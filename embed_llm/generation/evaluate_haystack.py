...
EVALUATOR_TEMPLATE = {"template":"""You are an expert grader of student answers relative to a reference answer. \n 
...
def get_viz(score_file_path: str):
    ...
    sorted_context_lengths = [str(ctx_l) for ctx_l in sorted([int(cl) for cl in context_lengths])]
    depth_percents = list(scores[temps[0]][context_lengths[0]].keys())
    sorted_depth_percents = [str(ctx_l) for ctx_l in sorted([int(dp) for dp in depth_percents])]
    for i, temp in enumerate(temps):
        for j, context_length in enumerate(context_lengths):
            for k, depth_percent in enumerate(depth_percents):
                data_scores[temp].append({"Document Depth": depth_percent, 
                                          "Context Length": context_length, 
                                          "Score": 1 if isinstance(scores[temp][context_length][depth_percent],str) else scores[temp][context_length][depth_percent]})
    ...
    for i, temp in enumerate(temps):
        ...
        # More aesthetics
        ax[i].set_title(f'Pressure Testing Llama 70B Context with temperature {temp}')  # Adds a title
        ax[i].set_xlabel('Token Limit')  # X-axis label
        ax[i].set_ylabel('Depth Percent')  # Y-axis label
        ax[i].xaxis.set_tick_params(rotation=45)  # Rotates the x-axis labels to prevent overlap
        ax[i].yaxis.set_tick_params(rotation=0)  # Ensures the y-axis labels are horizontal
    ...
    for temp in results:
        ...
        for context_length in results[temp]:
            ...
            for depth_percent in results[temp][context_length]:
                print(f"Temp: {temp}, Context Length: {context_length}, Depth Percent: {depth_percent}")
                if isinstance(results[temp][context_length][depth_percent], str):
                    ...
                    continue
                messages = [
                {"role": "system", "content": """ You are an expert grader of student answers relative to a reference answer. \n 
                        Your goal is to provide a numerical score between 0 and 10, where 0 indicates no similarity or correctness, and 10 indicates a perfect match according to the following criteria: \n
                        Score 1: The answer is completely unrelated to the reference.\n
                        Score 3: The answer has minor relevance but does not align with the reference.\n
                        Score 5: The answer has moderate relevance but contains inaccuracies.\n
                        Score 7: The answer aligns with the reference but has minor omissions.\n
                        Score 10: The answer is completely accurate and aligns perfectly with the reference.\n"""},
                {"role": "user", "content": ("""Here is the student answer: \n --- --- --- \n {answer}
                        Here is the reference answer: \n --- --- --- \n {reference}""").format(answer = generated, reference = needle)},    
                ]
                ouptputs = pipeline(messages, max_new_tokens=32)
                try:
                    score = re.findall(r'\b\d+\b',ouptputs[0]['generated_text'][-1]['content'].split('\n\n')[0])
                    if len(score) > 0:
                        scores[temp][context_length][depth_percent] = int(score[0])
                    else:
                        scores[temp][context_length][depth_percent] = ouptputs[0]['generated_text'][-1]['content'].split('\n\n')[0]
                except:
                    scores[temp][context_length][depth_percent] = ouptputs[0]['generated_text'][-1]['content'].split('\n\n')[0]
