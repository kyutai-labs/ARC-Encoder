...
def evaluate_toydecompression(
    ...
    data_path_minor_OOD = (
        "/lustre/scwpod02/client/kyutai-interns/datasets/finemath/train.jsonl"
    )
    ...
    if data == "random":
        ...
        data_path = (
            "/lustre/scwpod02/client/kyutai-interns/datasets/thestack/shuf/python.jsonl"
        )
    else:
    ...
    if Path(data_path).exists():
        ...
        for _ in range(n_samples):
            sample = pipeline.tokenizer.decode(
                [
                    random.randint(0, pipeline.tokenizer.n_words - 1)
    ...
    if decompress_task == "from_prefix_reconstruct":
        ...
        raise NotImplementedError(
            f"Decompression task {decompress_task} not implemented"
        )
    ...
    for i in trange(0, len(full_x), max_bs):
        ...
        embeddings = pipeline.model.pooling_module(
            x=embeddings.to(pipeline.pipeline_args.param_dtype), seqlens=seqlens
        )
        ...
        embeddings = (
            cat_embeddings if other_device is None else cat_embeddings.to(other_device)
        )

        with torch.no_grad():
            input = torch.tensor(
                sum(ppl_tok_input[i : i + max_bs], []), dtype=torch.long
            ).to(other_device)
            ...
            for toks in ppl_tok_output[i : i + max_bs]:
                loss_in_bits = torch.sum(
                    compute_bpt_loss(
                        logits_w_embeds[ind : ind + len(toks), ...],
                        torch.tensor(toks).to(other_device),
                        None,
                    )
                ).item()
                batch_bpc += loss_in_bits / len(pipeline.tokenizer.decode(toks))
                batch_ce += compute_ce_loss_with_mask(
                    logits_wo_embeds[ind : ind + len(toks), ...],
                    torch.tensor(toks).to(other_device),
                    None,
                ).item()
                batch_bpc_wo += torch.sum(
                    compute_bpt_loss(
                        logits_wo_embeds[ind : ind + len(toks), ...],
                        torch.tensor(toks).to(other_device),
                        None,
                    )
                ).item() / len(pipeline.tokenizer.decode(toks))
                ind += len(toks)
        ...
        produced_text = [
            pipeline.tokenizer.decode(generated_tokens[temp])
...
def arg_parser():
    ...
    parser.add_argument(
        "--out_file",
        type=str,
        default="/home/hippolytepilchen/code/embed_llm/results/toy_tests/decompression_tests_2.jsonl",
    )
