...
class LlamaCrossAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.k_proj = nn.Linear(
            input_hidden_size, self.num_key_value_heads * self.head_dim, bias=False
        )
        self.v_proj = nn.Linear(
            input_hidden_size, self.num_key_value_heads * self.head_dim, bias=False
        )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.num_cross_attn_hidden_states = getattr(
            config, "num_cross_attn_hidden_states", 1
        )
        ...
        for i in range(config.num_hidden_layers):
            config.do_cross_attention = (
                i >= config.num_hidden_layers - self.num_cross_attn_layers
            ) and self.do_cross_attention
        ...
        if encoder_hidden_states is not None:
            if encoder_attention_mask is None:
                encoder_attention_mask = torch.ones(
                    (batch_size, encoder_hidden_states[0].shape[1]),
                    dtype=torch.bool,
                    device=inputs_embeds.device,
                )
                encoder_padding_mask = None
        ...
        if self.gradient_checkpointing and self.training:
            ...
            if encoder_hidden_states is not None:
                encoder_hidden_states = [
                    x.requires_grad_(True) for x in encoder_hidden_states
                ]
            if use_cache:
                logger.warning_once(
            ...
            if encoder_hidden_states is not None and decoder_layer.do_cross_attention:
                ...
                encoder_hidden_state = (
                    ...
                    if self.num_cross_attn_hidden_states == 1
                        ...
                        max(idx - len(self.layers), -len(self.layers))
                    ]
                )
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning_once(
                "The LlamaForCausalLM model has config set to False, but setting it to True to get the expected behavior. If this is not intended, check the code."
            )
...
class LlamaForReplugCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config):
        ...
        self.replug_passage_temperature = getattr(
            config, "replug_passage_temperature", 1.0
        )
    ...
    def calculate_weighted_logits(self, logits, context_scores, bsz, n_ctx):
        ...
        weighted_prob = seq_prob.sum(
            dim=1, keepdim=True
        )  # shape is bsz, 1, seq_length, vocab_size
        log_prob = torch.log(weighted_prob)

        # log_prob is the weighted logits
        final_logits = (
            log_prob.expand(-1, n_ctx, -1, -1)
            .contiguous()
            .view(bsz * n_ctx, -1, vocab_size)
        )
        return final_logits
    ...
    def forward(
        ...
        assert context_scores.shape == (bsz, n_ctx), (
            "RePlug expects context_scores of shape (bsz, n_ctx)"
        )
        ...
        if self.separate_forward or self.lm_eval_mode:
            ...
            for i in range(len(input_ids)):
                pkvs = (
                    tuple(
                        (
                            k[i : i + 1].to(input_ids.device),
                            v[i : i + 1].to(input_ids.device),
                        )
                ...
                if not self.lm_eval_mode:
                    if outputs[1] is not None and len(outputs[1]) > 0:
                        all_past_kvs.append(
                            tuple((kv[0].cpu(), kv[1].cpu()) for kv in outputs[1])
                        )
                    if output_hidden_states:
            ...
            for i in range(bsz):
                ...
                # bsz = 1 here
                logits = self.calculate_weighted_logits(
                    logits, context_scores[i : i + 1], 1, n_ctx
                )
            ...
            if not self.lm_eval_mode:
                ...
                past_key_values = (
                    ...
                    if len(all_past_kvs) >= 1
                    else None
                )

                hidden_states = (
                    torch.cat(all_hidden_states, dim=0).to(input_ids.device)
                    if output_hidden_states
                    else None
                )
                attentions = (
                    torch.cat(all_attentions, dim=0).to(input_ids.device)
                    if output_attentions
                    else None
                )
...
class LlamaForCausalContextLM(LlamaForCausalLM):
    ...
    def __init__(self, config, encoder=None):
        ...
        if not config.is_decoder:
            logger.warning_once(
                "The LlamaForContextCausalLM model has config set to False, but setting it to True to get the expected behavior. If this is not intended, check the code."
            )
            config.is_decoder = True
        if self.encoder_is_model:
            ...
            if type(config.encoder_config) is dict:
    ...
    def set_encoder(self, encoder):
        if self.encoder_is_model:
            print(
                "warning: currently the encoder is set to self.model, ignoring the set"
            )
        else:
            self.encoder = encoder
    ...
    def forward(
        ...
        if encoder_hidden_states is None and encoder_input_ids is not None:
            ...
            if encoder_attention_mask is None:
                encoder_attention_mask = torch.ones_like(
                    encoder_input_ids, device=encoder_input_ids.device
                )
            ...
            train_encoder = (
                self.training and self.encoder is not None and self.train_encoder
            )
            ...
            if "with_query_no_hid" in self.encode_mode:
                ...
                encoder_hidden_states = [
                    x[:, :ctx_length] for x in encoder_hidden_states
                ]
            ...
            encoder_hidden_states = [
                x.reshape(bsz, n_ctx * ctx_length, -1) for x in encoder_hidden_states
            ]
            if self.offload_hidden_states:
                encoder_hidden_states = [
                    x.cpu() if i < len(encoder_hidden_states) - 1 else x
                    for i, x in enumerate(encoder_hidden_states)
                ]
            encoder_attention_mask = encoder_attention_mask.view(
                bsz, n_ctx * ctx_length
            )
        ...
        if self.train_batch_mode == "in_batch_negative" and self.training:
            ...
            encoder_hidden_states = [
                x.view(1, -1, x.size(-1)).expand(bsz, -1, -1)
        ...
        if distill_prob is not None and distill_index is not None:
            ...
            prob = F.softmax(
                logits[..., -distill_prob.size(1) :, :].contiguous(), dim=-1
            )
            ...
            together_prob = torch.cat([top_prob, other_prob.unsqueeze(2)], dim=2).view(
                -1, distill_prob.size(1)
            )

            if "smooth" in self.kl_loss_mode:
                delta = float(self.kl_loss_mode.split("_")[-1])
                # smoothing to avoid log(0)
                distill_prob = (distill_prob + delta) / (
                    1 + delta * distill_prob.size(1)
                )
                together_prob = (together_prob + delta) / (
                    1 + delta * together_prob.size(1)
                )
