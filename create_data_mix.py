...
def main(args):
    ...
    if args.train_files is not None:
        ...
        with open(args.folder_path + args.output_file, 'a') as f:
            for file, weight in zip(args.train_files.split(','), args.train_weights.split(',')):
                f.write(json.dumps({'train_data': {'path': file, 'weight': weight}}) + '\n')
    ...
    if args.eval_files is not None:
        ...
        with open(args.folder_path + args.output_file, 'a') as f:
            for file, weight in zip(args.eval_files.split(','), args.eval_weights.split(',')):
                f.write(json.dumps({'eval_data': {'path': file, 'weight': weight}}) + '\n')
